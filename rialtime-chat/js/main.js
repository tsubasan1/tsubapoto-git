...
const toggleFavorite = (e) => {
  ...
  if (dbdata.favorites && dbdata.favorites[messageId]) {
    ...
    firebase
      .database()
      .ref(`favorites/${currentUID}/${messageId}`)
      .set({message,createdAt: firebase.database.ServerValue.TIMESTAMP});
  };
};
// お気に入り一覧のモーダルを初期化
...
const createMessageDiv = (messageId, message) => {
  ...
  if (dbdata.favorites && dbdata.favorites[messageId]) {
    (mfl).find("i").removeClass('fa-star-o');
    (mfl).find("i").addClass('fa-star');
  }
...
const loadChatView = () => {
  ...
  favoritesRef.on('child_removed', (favSnapshot) => {
    ...
    // お気に入り一覧モーダルから該当のお気に入���情報を削除する
    $(`#favorite-message-id-${messageId}`).remove();
    
    
    // TODO: お気に入りリンクのアイコンを、塗りつぶしなし(fa-star-o) に変更する
    $(`#message-id-${messageId}`).find("i").removeClass('fa-star');
    $(`#message-id-${messageId}`).find("i").addClass('fa-star-o');
  ...
  favoritesRef.on('child_added', (favSnapshot) => {
    ...
    $(`#message-id-${messageId}`).find("i").removeClass('fa-star-o');
    $(`#message-id-${messageId}`).find("i").addClass('fa-star');
